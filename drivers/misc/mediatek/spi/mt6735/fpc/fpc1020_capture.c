...
int fpc1020_capture_task(fpc1020_data_t *fpc1020)
{
	...
	if (mode == FPC1020_MODE_SINGLE_CAPTURE_CAL) {
		...
		if (error){
			printk("[FPC] fpc1020_capture_task line=%d, error=%d\n",__LINE__,  error);
			goto out_error;
		}
	} else if (mode == FPC1020_MODE_CAPTURE_AND_WAIT_FINGER_UP &&
		  fpc1020->chip.type == FPC1020_CHIP_1025XG) {
		error = fpc1020_capture_set_sample_mode(fpc1020, 8);
		if (error)
			{
			printk("[FPC] fpc1020_capture_set_sample_mode line=%d, error=%d\n",__LINE__,  error);
			goto out_error;
			}
	}
