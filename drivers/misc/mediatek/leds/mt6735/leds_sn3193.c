...
#define GPIO_LED_EN		(GPIO131 | 0x80000000)
...
struct platform_device sn3193_leds_platform_device = {
    .name   = "leds-sn3193",
    .id     = -1,
};
...
{
	LEDS_DEBUG("[LED]%s delay_on=0x%lx, delay_off=0x%lx\n", __func__,*delay_on,*delay_off);
...
static void sn3193_proc_backlight_value(int level, int delay_on, int delay_off)
{
	...
	Rt2=(delay_on>>SN3193_BL_RT2_OFFSET_BIT)&SN3193_BL_T2_MASK;// wuwl10    (delay_on >>0) & 0x0F
	if(Rt2>8) Rt2=8;
	Rt3=(delay_on>>SN3193_BL_RT3_OFFSET_BIT)&SN3193_BL_T3_MASK;// wuwl10    (delay_on >>4) & 0x07
	Rt1=(delay_on>>SN3193_BL_RT1_OFFSET_BIT)&SN3193_BL_T1_MASK;// wuwl10    (delay_on >>7) & 0x07
	...
	LEDS_DEBUG("[LED]%s cur:%s rgb:0x%x 0x%x 0x%x\n; R:t0=%s t1=%s t2=%s t3=%s t4=%s; G:t0=%s t1=%s t2=%s t3=%s t4=%s; B:t0=%s t1=%s t2=%s t3=%s t4=%s;\n",
				__func__,
				cur_text[led_cur],
...
static int __init sn3193_leds_platform_probe(struct platform_device *pdev)
{
	...
	g_sn3193_leds_data[1]->gpio = GPIO_LED_EN;//GPIO_LED_EN;
...
static int __init sn3193_leds_init(void)
{
	...
	ret = platform_device_register(&sn3193_leds_platform_device);
	if (ret) {
		printk("[LED]%s:dev:E%d\n", __func__,ret);
		return ret;
	}    
