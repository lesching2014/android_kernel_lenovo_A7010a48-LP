...
WLAN_STATUS nicTxPollingResource(IN P_ADAPTER_T prAdapter, IN UINT_8 ucTC)
{
    ...
    /*Begin, lenovo-sw lumy1 workaround for CAP-4236*/

    if(u4Status == WLAN_STATUS_FAILURE) {
	HAL_SET_FLAG(prAdapter, ADAPTER_FLAG_FW_ERR);
    }
...
#if DBG
    {
        ...
        if (i4Times) {
            DBGLOG(TX, ERROR, ("Polling MCR_WTSR delay %ld times, %ld msec\n",
					   i4Times,
					   (i4Times * NIC_TX_RESOURCE_POLLING_DELAY_MSEC)));
        }
    }
#endif /* DBG */
    /*End, lenovo-sw lumy1 workaround for CAP-4236*/
...
nicTxIsTXDTemplateAllowd(IN P_ADAPTER_T prAdapter, 
    ...
    if(prMsduInfo->fgIsTXDTemplateValid && (prMsduInfo->ucControlFlag == 0) 
        ...
        /*Begin, lenovo-sw lumy1, send DHCP and ARP with highest priority and fix tx rate, CAP-3850*/
	if (!GLUE_TEST_PKT_FLAG(prMsduInfo->prPacket, ENUM_PKT_ARP) &&
	    	!GLUE_TEST_PKT_FLAG(prMsduInfo->prPacket, ENUM_PKT_DHCP)) {
	    	DBGLOG(INIT, INFO, ("don't use TXD Template for arp/dhcp\n"));
			return FALSE;
	}
	/*End, lenovo-sw lumy1, send DHCP and ARP with highest priority and fix tx rate, CAP-3850*/
        return TRUE;
    }
    else {
        return FALSE;
		  ...
		  IN P_MSDU_INFO_T prMsduInfo, IN P_NATIVE_PACKET prPacket)
{
	...
	if ((GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_DHCP) || GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_ARP)
		||GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_1X)) && IS_BSS_INDEX_VALID(prMsduInfo->ucBssIndex) ) {
			P_BSS_INFO_T prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, prMsduInfo->ucBssIndex);
			prMsduInfo->u4FixedRateOption |=
				((prBssInfo->u2HwDefaultFixedRateCode <<2  ) & TX_DESC_FIXDE_RATE_MASK) <<  16;
			prMsduInfo->ucRateMode = MSDU_RATE_MODE_MANUAL_DESC;
			prMsduInfo->ucUserPriority = TX_DESC_TID_NUM - 1;
			DBGLOG(INIT, INFO, ("using fix rate %d and TID %d for arp/dhcp/eap\n", prBssInfo->u2HwDefaultFixedRateCode, TX_DESC_TID_NUM - 1));
	}
	 /*End, lenovo-sw lumy1, send DHCP and ARP with highest priority and fix tx rate, CAP-3850*/
