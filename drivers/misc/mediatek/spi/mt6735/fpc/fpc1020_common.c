...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1025XG, 0xB210, 1, FPC1021_ROWS, FPC1021_COLUMNS, FPC102X_ADC_GROUP_SIZE,1600, 1000},
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	if (error)
		return error;
#ifdef DEBUG_LOG
	printk("fpc Hardware id, detected 0x%x - forced setting 0x%x\n",
					hardware_id, fpc1020->force_hwid);
#endif

	//fpc1020->force_hwid = 0xB210;
	...
	if (match) {
		...
		printk("fpc type=%d, revision=%d, hardware_id=%d\n", fpc1020->chip.type, fpc1020->chip.revision, hardware_id);
		/*dev_err(&fpc1020->spi->dev,
...
#ifdef DEBUG_LOG
	printk("%s---fpc--Hardware id: 0x%x (type=%d; rev.%d) \n",
