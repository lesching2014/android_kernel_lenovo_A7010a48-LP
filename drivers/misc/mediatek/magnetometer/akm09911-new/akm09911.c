...
/*lenovo-sw caoyi1 modify magnetometer sensor data 20150825 begin*/
	*status = sensor_data[8];
/*lenovo-sw caoyi1 modify magnetometer sensor data 20150825 end*/
...
/*lenovo-sw caoyi1 add for GPIO reset pin 20150320 begin*/
	printk("%s:akm09911 reset pin is used for this project>>>>\n",__func__);
	mt_set_gpio_mode(GPIO_MSENSOR_RST_PIN, GPIO_MSENSOR_RST_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_MSENSOR_RST_PIN, GPIO_DIR_OUT);
	//mt_set_gpio_out(GPIO_MSENSOR_RST_PIN,0);

	//mdelay(125);
	mt_set_gpio_out(GPIO_MSENSOR_RST_PIN, 1);

	printk("%s:akm09911 reset pin is used for this project\<<<<n",__func__);

/*
#ifdef GPIO_MSENSOR_RST_PIN
	printk("%s:akm09911 reset pin is used for this project\n",__func__);
	err = mt_set_gpio_mode(GPIO_MSENSOR_RST_PIN, GPIO_MSENSOR_RST_PIN_M_GPIO);
	if(err < 0)
	{
		printk(KERN_ERR "set gpio mode error\n");
	}
	err = mt_set_gpio_dir(GPIO_MSENSOR_RST_PIN, GPIO_DIR_OUT);
	if(err < 0)
	{
		printk(KERN_ERR "set gpio dir error\n");
	}
	err = mt_set_gpio_out(GPIO_MSENSOR_RST_PIN,0);
	if(err < 0)
	{
		printk(KERN_ERR "set gpio out value error\n");
	}
	mdelay(125);
	err = mt_set_gpio_out(GPIO_MSENSOR_RST_PIN, 1);
	if(err < 0)
	{
		printk(KERN_ERR "set gpio out value error\n");
	}
#endif
*/
/*lenovo-sw caoyi1 add for GPIO reset pin 20150320 end*/
