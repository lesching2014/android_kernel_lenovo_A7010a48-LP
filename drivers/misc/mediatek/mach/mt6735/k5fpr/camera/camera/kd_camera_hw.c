...
u32 pinSet[3][8] = {
    ...
    if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K3M2_MIPI_RAW,currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K3M2_SUNNY_MIPI_RAW,currSensorName))))
        {
         //AF>MCLK>AVDD>delay 0ns->DOVDD->>delay 0ns->DVDD->>delay 0ns->RESET->>delay 10us>IIC
            //First Power Pin low and Reset Pin Low
            //lenovo.sw wangsx3  change for K5,according to HW owner
            ...
            mt_set_gpio_mode(CAMERA_MCLK0,1);
            mdelay(3);
            //ISP_MCLK1_EN(TRUE);
            //mdelay(1);
            ...
            //mdelay(1);
            #if 1 // k5 use xshutdown pin 
            // PWDN 
            if(pinSetIdx == 0)
            {
              mt_set_gpio_mode(CAMERA_CMPDN_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMPDN_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMPDN_PIN, GPIO_OUT_ONE);				    
              mdelay(2);	
            }
            else
            {
              mt_set_gpio_mode(CAMERA_CMPDN_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMPDN_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMPDN_PIN, GPIO_OUT_ZERO);				    
              mdelay(2);	
            }
	#endif
            // xshutdown,,reset 
            if(pinSetIdx == 0)
            {
              mt_set_gpio_mode(CAMERA_CMRST_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMRST_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMRST_PIN, GPIO_OUT_ONE);				    
              //mdelay(2);	
            }
            else
            {
              mt_set_gpio_mode(CAMERA_CMRST_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMRST_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMRST_PIN, GPIO_OUT_ZERO);				    
              //mdelay(2);	
            }
        ...
        else if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV5693_MIPI_RAW, currSensorName))||(0 == strcmp(SENSOR_DRVNAME_OV5693_OFILM_MIPI_RAW, currSensorName))))
        {
        //lenovo.sw wangsx3 Power on:AVDD>0ns>DOVDD>=Reset>0ns>DVDD>0ns>PWDN>0.4ms>MCLK
            //First Power Pin low and Reset Pin Low
            //mclk>AVDD>0ns>DOVDD>=reset>0ns>DVDD>0ns>PWN
           //mdelay(1);
           ...
           //pull  up camera_sw_select  pin 
            //mt_set_gpio_mode(GPIO_CAM_SWSEL, 0);
            //mt_set_gpio_dir(GPIO_CAM_SWSEL, GPIO_DIR_OUT);
            //mt_set_gpio_out(GPIO_CAM_SWSEL, GPIO_OUT_ONE);
            //VCAM_A
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A), power id = %d\n", CAMERA_POWER_VCAM_A);
            ...
            mdelay(1);
           //pull  up RESET pin 
            mt_set_gpio_mode(CAMERA_CMRST1_PIN, 0);
            mt_set_gpio_dir(CAMERA_CMRST1_PIN, GPIO_DIR_OUT);
            mt_set_gpio_out(CAMERA_CMRST1_PIN, GPIO_OUT_ONE);	
            mdelay(2);
           //pull  up DVDD pin 
            ...
            if(pinSetIdx == 1)
            {
              mt_set_gpio_mode(CAMERA_CMPDN1_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMPDN1_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMPDN1_PIN, GPIO_OUT_ONE);				    
              //mdelay(2);	
            }
            else
            {
              mt_set_gpio_mode(CAMERA_CMPDN1_PIN, 0);
              mt_set_gpio_dir(CAMERA_CMPDN1_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMPDN1_PIN, GPIO_OUT_ZERO);				    
             // mdelay(2);	
            }
            //start---------------enable inactive camera----------------------------,lenovo.sw wangsx3 add for K5
            //main camemra DVDD
            //mdelay(1);
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1000,mode_name))
            {
            ...
            //pull down reset pin
              ...
              mt_set_gpio_dir(CAMERA_CMRST_PIN, GPIO_DIR_OUT);
              mt_set_gpio_out(CAMERA_CMRST_PIN, GPIO_OUT_ZERO);	
            //end---------------enable inactive camera----------------------------,lenovo.sw wangsx3 add for K5
    ...
    if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K3M2_MIPI_RAW,currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K3M2_SUNNY_MIPI_RAW,currSensorName))))
        {
         //AF>MCLK->RESET>AVDD>delay 0ns->DOVDD->>delay 0ns->DVDD->>delay 0ns->>delay 10us>IIC
            //AF_VCC
            ...
            ISP_MCLK1_EN(FALSE);
            mt_set_gpio_mode(CAMERA_MCLK0, 0);
            mt_set_gpio_dir(CAMERA_MCLK0, GPIO_DIR_OUT);
            mt_set_gpio_out(CAMERA_MCLK0, GPIO_OUT_ZERO);
            mdelay(1);
	  ...
	  //DVDD
        ...
        {
         //lenovo.sw wangsx3  Power down:MCLK>PWDN>DVDD>>Reset>DOVDD>AVDD
           //pull  down camera_sw_select  pin 
           // mt_set_gpio_mode(GPIO_CAM_SWSEL, 0);
          //  mt_set_gpio_dir(GPIO_CAM_SWSEL, GPIO_DIR_OUT);
          //  mt_set_gpio_out(GPIO_CAM_SWSEL, GPIO_OUT_ZERO);
           ...
           //pull  down RESET pin 
            mt_set_gpio_mode(CAMERA_CMRST1_PIN, 0);
            mt_set_gpio_dir(CAMERA_CMRST1_PIN, GPIO_DIR_OUT);
            mt_set_gpio_out(CAMERA_CMRST1_PIN, GPIO_OUT_ZERO);				
            //VCAM_IO,DOVDD
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_IO, mode_name)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d \n", CAMERA_POWER_VCAM_IO);
                //return -EIO;
            ...
            //mt_set_gpio_dir(GPIO_CAMERA_SUB_AVDD, GPIO_DIR_OUT);
            //mt_set_gpio_out(GPIO_CAMERA_SUB_AVDD, GPIO_OUT_ZERO);
