...
#include "ov5693mipi_ofilm_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV5693_ofilm_camera_sensor"
#define LOG_1 LOG_INF("OV5693,OFILM MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5693_OFILM_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.i2c_addr_table = {0x20,0xff},//record sensor support all write id addr, only supprt 4must end with 0xff.//lenovo.sw wangsx3 change for K5
    .i2c_speed = 300,
};
...
#define BG_Ratio_Typical 0x11F  //lenovo.sw wangsx3 change for K5
#define RG_Ratio_Typical 0x10C

#define MID_ADDR 0x3D01
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	//LOG_INF("-----temp=0x%x, (*otp_ptr).light_rg=0x%x, (*otp_ptr).light_bg=0x%x\n",temp,(*otp_ptr).light_rg , (*otp_ptr).light_bg);
	//LOG_INF("-----6=0x%x, 7=0x%x, 8=0x%x  9=0x%x\n", read_cmos_sensor(address + 6), read_cmos_sensor(address +7) ,  read_cmos_sensor(address +8 ),read_cmos_sensor(address +9));
	// clear otp buffer
	for (i=0;i<16;i++) {
		write_cmos_sensor(0x3d00 + i, 0x00);
	}
...
// otp_ptr: pointer of otp_struct
// return:            0,
static int read_otp_lenc(int index)
{
	int bank, i;
	int address;
	// select bank: 4, 8, 12
...
static int  Read_otp_MID(void)
{
	...
	otp_index = 0xc0 | otp_index;
	write_cmos_sensor(0x3d84, otp_index);
	// read otp into buffer
	write_cmos_sensor(0x3d81, 0x01);
	mdelay(5);
	mid = read_cmos_sensor(MID_ADDR);
	
	LOG_ERR("[wuyt3]Read_otp_MID Read MID,mid=0x%x\n", mid);
...
LOG_INF("E\n");
	...
	kal_uint16 init_para[]={
		...
		0x5e10,0x0c,//);
		//START lenovo.sw wangsx3 add for low light environment,autolenc switch enable
		0x5841,0x0d,//);
		0x583e,0x10,//);
		0x583f,0x0a,//);
		0x5840,0x18,//);
		//END lenovo.sw wangsx3 add for low light environment,autolenc switch enable
		0x0100,0x01,//);  //wake up	 
	};
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = *sensor_id + mid;
			...
			LOG_ERR("Read sensor id fail, write id: 0x%x, read sensor id = 0x%x ,imgsensor_info.sensor_id=0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 OV5693_OFILM_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
