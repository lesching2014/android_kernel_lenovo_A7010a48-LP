                                ...
                                const char *buf, size_t len)
{
    ...
    if(!attribute->store) 
	return -EIO;
		printk("Horizon: swtp_attr_store(), buf = %s, len = %d\n", buf, (int)(len));
...
//linyf modify for SWTP func 2015-07-28 begin
//change by jianjun
/*
...
extern	unsigned int swtp_get_mode();
extern  int swtp_set_mode(unsigned int ctrid, unsigned int enable);
                               ...
                               char *buf)
{
	...
	run_mode = swtp_get_mode();
	
	buf_len += sprintf(buf, "run_mode: %d\n", run_mode);
	return buf_len;
}
                                ...
                                const char *buf, size_t len)
{
	...
	set_mode = buf[0]-'0';
	printk("Horizon, before swtp_sysfs_store, set_mode=%d\n", set_mode);
	swtp_set_mode(0, set_mode);
	printk("Horizon, after swtp_sysfs_store, set_mode=%d\n", set_mode);
	return len;
}
...
//end jianjun
//linyf modify for SWTP func 2015-07-28 end
