...
#if 1 /*SPI*/
static struct regulator_consumer_supply spi_dc1v8_consumers[] = {
	REGULATOR_SUPPLY("AVDD", "spi0.0"), 
	REGULATOR_SUPPLY("DBVDD1", "spi0.0"), 
	REGULATOR_SUPPLY("LDOVDD", "spi0.0"),
	REGULATOR_SUPPLY("DBVDD2", "florida-codec"),
	REGULATOR_SUPPLY("DBVDD3", "florida-codec"),
	REGULATOR_SUPPLY("CPVDD",  "florida-codec"),
	REGULATOR_SUPPLY("SPKVDDL", "florida-codec"),
	REGULATOR_SUPPLY("SPKVDDR", "florida-codec"),
	REGULATOR_SUPPLY("CPVDD", "spi0.0"),
};
#endif

static struct regulator_init_data spi_dc1v8_data = {
	.constraints = {
		.always_on = 1,
	},
	.num_consumer_supplies = ARRAY_SIZE(spi_dc1v8_consumers),
	.consumer_supplies = spi_dc1v8_consumers,
};

static struct fixed_voltage_config spi_dc1v8vdd_pdata = {
	.supply_name = "DC_1V8",
	.microvolts = 1800000,
	.init_data = &spi_dc1v8_data,
	.gpio = -1,
};
static struct mfd_cell spi_dc1v8_regulators[] = {
    [0]={
        .name = "reg-fixed-voltage",
        .platform_data = &spi_dc1v8vdd_pdata,
        .pdata_size = sizeof(spi_dc1v8vdd_pdata)
    },
};
/* lenovo-sw zhangrc2 compatible gpio-spi and spi */
static int arizona_of_init_mtk_gpio(struct arizona *arizona)
{
	if (arizona->pdata.reset) {
		mt_set_gpio_mode(GPIO_ARIZONA_RST_PIN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_ARIZONA_RST_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_ARIZONA_RST_PIN, GPIO_OUT_ONE);
	}
	
	if (arizona->pdata.ldoena) {
		mt_set_gpio_mode(GPIO_ARIZONA_LDO_ENABLE, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_ARIZONA_LDO_ENABLE, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_ARIZONA_LDO_ENABLE, GPIO_OUT_ZERO);
	}
	/*IRQ should have some special configure for MTK*/
	if (arizona->pdata.irq_gpio) {
		#if 0
		mt_set_gpio_mode(GPIO_ARIZONA_IRQ_GPIO,GPIO_MODE_01);
		mt_set_gpio_dir(GPIO_ARIZONA_IRQ_GPIO, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(GPIO_ARIZONA_IRQ_GPIO, GPIO_PULL_DISABLE);
		if(arizona->pdata.irq_arizona_mtk_delay)
		mt_gpio_set_debounce(GPIO_ARIZONA_IRQ_GPIO,arizona->pdata.irq_arizona_mtk_delay);
		#endif
	}
	return 0;
}

static int arizona_mtk_gpio_output(int arizona_gpio,int Level)
{
	printk("%s arizona_gpio =%d  Level=%d \n", __func__,arizona_gpio,Level);
	mt_set_gpio_out(arizona_gpio|0x80000000, Level);
	return 0;
}
...
static int arizona_runtime_resume(struct device *dev)
{
	...
	dev_err(arizona->dev, "Leaving AoD mode\n");

	switch (arizona->type) {
	case WM5110:
	case WM8280:
		if (arizona->rev == 3 && arizona->pdata.reset){
			if(arizona->pdata.gpio_mtk_mode){
		 	 	arizona_mtk_gpio_output(arizona->pdata.reset,GPIO_OUT_ZERO);
			}
			else{
	...
	if((arizona->pdata.gpio_mtk_mode)&&(arizona->pdata.ldoena))
 	{
 	  arizona_mtk_gpio_output(arizona->pdata.ldoena,GPIO_OUT_ONE);
      msleep(5); 
 	}
	...
	case WM8280:
		if (arizona->rev == 3) {
			if (!arizona->pdata.reset) {
				...
				if(arizona->pdata.gpio_mtk_mode){
		 	 	arizona_mtk_gpio_output(arizona->pdata.reset,GPIO_OUT_ONE);
				}
				else{
...
err:
	...
	if((arizona->pdata.gpio_mtk_mode)&&(arizona->pdata.ldoena))
 	{
 	  arizona_mtk_gpio_output(arizona->pdata.ldoena,GPIO_OUT_ZERO);
 	}
...
static int arizona_runtime_suspend(struct device *dev)
{
    ...
    printk("Entering AoD mode %s\n",__func__); 

	dev_err(arizona->dev, "Entering AoD mode\n");
	switch(arizona->type) {
	case WM5102:
	case WM8997:
	case WM8998:
...
/* lenovo-sw zhangrc2 compatible gpio-spi and spi */
	if (audio_hardid ==1) {
         	/*Add by karl for regulator register*/
		ret = mfd_add_devices(arizona->dev, -1, spi_dc1v8_regulators,
                	ARRAY_SIZE(spi_dc1v8_regulators), NULL, 0,NULL);
		printk ("shone audio hardid is high\n");	
	} else {
		ret = mfd_add_devices(arizona->dev, -1, dc1v8_regulators,
                	ARRAY_SIZE(dc1v8_regulators), NULL, 0,NULL);
		printk ("shone audio hardid is low\n");	
	} 
/* lenovo-sw zhangrc2 compatible gpio-spi and spi */
	if (ret != 0) {
    ...
    printk("arizona in %s arizona->pdata.gpio_mtk_mode=%d\n", __func__,arizona->pdata.gpio_mtk_mode);
	...
	default:
		dev_err(arizona->dev, "Unknown device ID: %x\n", reg);
                printk("karl in %s Unknown device ID: %x\n", __func__,reg);
		goto err_reset;
	}
...
/*remove here because the chip will power down */
