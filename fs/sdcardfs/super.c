...
static int sdcardfs_show_options(struct seq_file *m, struct dentry *root)
{
    ...
    if (opts->lower_fs == LOWER_FS_EXT4) {
        seq_printf(m, ",lower_fs=ext4");
    } else if (opts->lower_fs == LOWER_FS_FAT) {
        seq_printf(m, ",lower_fs=fat");
    } else if (opts->lower_fs == LOWER_FS_F2FS) {
        seq_printf(m, ",lower_fs=f2fs");
    }
	return 0;
};
