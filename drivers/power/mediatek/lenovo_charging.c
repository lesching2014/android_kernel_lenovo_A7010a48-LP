...
//lenovo-sw zhangrc2 add for show codec id id Begin
#define LENOVO_SHOW_CODEC_IC_ID
//lenovo-sw zhangrc2 add for show codec id id Begin
...
#elif defined (LENOVO_USE_PMIC_CONTROL_LED )
 void lenovo_battery_charging_set_led_state(void) 
{
	if(charging_led_in_factory_mode != 1 )
	{
		if(BMT_status.UI_SOC==100)
		{
			mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,0,200);   
		}
		else
		{	
			 if(( BMT_status.charger_exist == KAL_TRUE ) && (charging_led_flag == 0) 
			 	...
			 	&&   (lenovo_chg_state != CHARGING_STATE_ERROR))
			{
				mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,200,200);   
			}
			else
			{
				mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,0,200);   
...
if (g_charger_test)	
{	
  msleep(5);	//reduce ui sync time 
  g_charger_test = 0;	
  printk( " g_charger_test first in \n");	
}	
...
#if defined(LENOVO_SHOW_UI_IN_BAT_OVERDISCHARGE_SUPPORT) 
...
static DEVICE_ATTR(charging_led_state, S_IRUGO|S_IWUSR, NULL, chg_store_charging_led_state);
	...
	if(charging_led_in_factory_mode != 1)
	{
		if (charging_led_flag == 1) {
			mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,0,200);
		  	     ...
		  	     && (BMT_status.UI_SOC !=100)  && (BMT_status.bat_charging_state != CHR_ERROR)
		  	    &&  (lenovo_chg_state != CHARGING_STATE_ERROR) ) {
		            mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,200,200);
		 } else {
			mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,0,200);
...
static DEVICE_ATTR(charging_ic_id, S_IRUGO|S_IWUSR, chg_show_charging_ic_id, NULL);
#endif
//lenovo-sw mahj2 add for show charging ic id End
//lenovo-sw zhangrc2 add for show codec id Begin
#ifdef LENOVO_SHOW_CODEC_IC_ID
extern int codec_id;
static ssize_t chg_show_codec_ic_id(struct device* dev,
				struct device_attribute *attr, char* buf)
{
	int florada_codec_id = 0;
    florada_codec_id = codec_id;
    return sprintf(buf, "0x%x\n", florada_codec_id);

}
static DEVICE_ATTR(codec_ic_id, S_IRUGO|S_IWUSR, chg_show_codec_ic_id, NULL);
#endif
//lenovo-sw zhangrc2 add for show codec id end
...
/* lenovo-sw zhangrc2 use pmic to control charging led 2014-12-08 */
#ifdef  LENOVO_USE_PMIC_CONTROL_LED  
	if (ret = device_create_file(dev, &dev_attr_charging_led_in_factory_mode))
	{
		return ret;
	}
	
	if (ret = device_create_file(dev, &dev_attr_charging_led_state))
	{
		return ret;
	}
	
   	if (ret = device_create_file(dev, &dev_attr_charging_led_flag))
	{
		return ret;
	}
#endif
...
//lenovo-sw mahj2 add for show charging ic id Begin
#ifdef LENOVO_SHOW_CHARGING_IC_ID
	if (ret = device_create_file(dev, &dev_attr_charging_ic_id))
	{
		return ret;
	}
#endif
//lenovo-sw mahj2 add for show charging ic id End
//lenovo-sw zhangrc2 add for show codec ic id begin
#ifdef LENOVO_SHOW_CODEC_IC_ID
	if (ret = device_create_file(dev, &dev_attr_codec_ic_id))
	{
		return ret;
	}
#endif
//lenovo-sw zhangrc2 add for show codec ic id End
