...
static int contain_appid_key(struct packagelist_data *pkgl_dat, void *appid) {
    ...
    /*lenovo-sw jixj 2015.2.13 modified begin for appid isn't in packages.list but can pass*/
    #if 0
    ...
    #endif
    /*lenovo-sw jixj 2015.2.13 modified end*/
	return 0;
}
...
static int read_package_list(struct packagelist_data *pkgl_dat) {
	...
	int fd;
	int read_amount;
    /*lenovo-sw jixj 2015.2.13 add begin*/
    bool is_found = false;
    /*lenovo-sw jixj 2015.2.13 add end*/
	...
	while ((read_amount = sys_read(fd, pkgl_dat->read_buf,
		...
		if (sscanf(pkgl_dat->read_buf, "%s %ld %*d %*s %*s %s",
            ...
            /*lenovo-sw jixj 2015.2.13 add begin, if gids!=write_gid,insert 0 to pkgl_dat*/
            if (is_found == false) {
                struct hashtable_entry *hash_cur;
                ...
                if(!contain_key) {
                    insert_int_to_null(pkgl_dat, (void *)appid, 0);;
                }
            }
            /*lenovo-sw jixj 2015.2.13 add end*/
		}
