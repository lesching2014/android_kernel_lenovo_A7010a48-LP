...
/*Begin, lenovo-sw lumy1 workaround for CAP-4236*/
	    DBGLOG(INIT, INFO, ("prAdapter->u4HwFlags %x\n",prAdapter->u4HwFlags));

		if (HAL_TEST_FLAGS(prAdapter, ADAPTER_FLAG_HW_ERR|ADAPTER_FLAG_FW_ERR) == TRUE) {
	    ...
	    DBGLOG(INIT, INFO, ("force firmware reset via software interrupt\n"));

            /* force release firmware own */
            kalDevRegWrite(prAdapter->prGlueInfo, MCR_WHLPCR, WHLPCR_FW_OWN_REQ_SET);

	    HAL_CLEAR_FLAG(prAdapter, ADAPTER_FLAG_HW_ERR|ADAPTER_FLAG_FW_ERR);
        }
