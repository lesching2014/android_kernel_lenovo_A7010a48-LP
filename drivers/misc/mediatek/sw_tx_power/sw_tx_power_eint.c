...
//linyf modify for SWTP func 2015-07-28
...
static void swtp_eint_2_handler(void)
{
    ...
    printk("[swtp]: rfcable_2_enable: %d\n", rfcable_enable);
...
int swtp_mod_eint_read_lenovo(void)
{
	if(mt_get_gpio_in(SWTP_GPIO_EINT_PIN) == SWTP_TRIGGERING)
	{
		printk("[swtp] cable in! [1]\n");
		return 1;
	}
	else
	{
		printk("[swtp] cable out! [0]\n");
		return 0;
	}
}
