...
static void arizona_extcon_set_mode(struct arizona_extcon_info *info, int mode)
{
	...
	{
		if(arizona->pdata.gpio_mtk_mode)/*florida gpio driver can't hook to mtk, write registers directory*/
		{
		 printk("arizona switch info->micd_modes[mode].gpio=%d \n",info->micd_modes[mode].gpio); 
		 if(info->micd_modes[mode].gpio)
		     regmap_write(arizona->regmap,ARIZONA_GPIO5_CTRL, 0x2901);
		 else
		 	 regmap_write(arizona->regmap,ARIZONA_GPIO5_CTRL, 0x2101);
		}
		else
		{
...
{
 /*the following code to check the headset of amereica and europe mode */
    static int currentBiasSelect=1;
	int SetrightBias=0;   
	if(currentBiasSelect!=info->micd_modes[info->micd_mode].bias)  
	{
		SetrightBias=currentBiasSelect;
		currentBiasSelect= info->micd_modes[info->micd_mode].bias;	
	}
	else
	{
		SetrightBias=info->micd_modes[info->micd_mode].bias;
	}
...
{      
	info->micd_low_ohm_done = false;
	 /*lenovo-sw zhangrc2 resolve headphone silence when remove and plug 2015-8-21 */
	arizona_extcon_hp_clamp(info, false);
	  /*lenovo-sw zhangrc2 resolve headphone silence when remove and plug 2015-8-21 */
				       ...
				       int val)
{
	struct arizona *arizona = info->arizona;
	int ret;
	dev_info(arizona->dev, "%s: Reading: %d\n", __func__, val);
	if (val < 0)
		return val;
	ret = arizona_micd_button_debounce(info, val);
	if (ret < 0)
...
static int arizona_antenna_oc_reading(struct arizona_extcon_info *info, int val)
{
	struct arizona *arizona = info->arizona;
	int ret;
	dev_info(arizona->dev, "%s: Reading: %d Ohms\n", __func__, val);
	if (val < 0)
		return val;
	ret = arizona_micd_button_debounce(info, val);
	if (ret < 0)
...
static int arizona_antenna_button_start(struct arizona_extcon_info *info)
{
	...
	for (i = 0; i < info->num_micd_ranges; i++)
		dev_info(arizona->dev, "%s: micd_lvl=%d: key=%d\n", __func__ ,
...
int arizona_micd_mic_reading(struct arizona_extcon_info *info, int val)
{
	...
	if( info->micd_low_ohm && !info->micd_low_ohm_done) {
		dev_info(arizona->dev, "Starting Low-Ohm-detection \n");   
	...
	if(val < 10)
	{
	       ...
	       dev_info(arizona->dev, "Detected OMTP headset\n");

done:
	pm_runtime_mark_last_busy(info->dev);

	/* Improve cross-talk */
#if 0 /*remove for delete mic polarity detect*/		
	if (!info->mic) {
		regmap_update_bits(arizona->regmap,
				ARIZONA_ACCESSORY_DETECT_MODE_1,
				ARIZONA_ACCDET_SRC,
...
static irqreturn_t arizona_micdet(int irq, void *data)
{
	...
	if (!info->detecting)
		debounce = 0;
	        ...
	        //patch: workaround solution is add mic detect delay and wait for devices ready.
	switch (arizona->type) {
	case WM8280:
	case WM5110:
		if (arizona->rev < 6) {
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	if (present) {
		...
		dev_info(arizona->dev, "Detected jack removal, val %d\n", val);
