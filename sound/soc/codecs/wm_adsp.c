...
#define WM_PAGE_SIZE_16K 1018*16             
/* lenovo-sw zhangrc2 compatible gpio-spi and spi */
struct wm_adsp_buf {
...
//    printk("arizona %s,%d,adsp->running=%d\n",__func__,__LINE__,adsp->running);
	mutex_lock(&ctl->lock);
...
#ifdef PWM_SPI_DEBUG
    t1=sched_clock();
    printk("arizona The time pwm dw firmware t1-t0=%llu\n",t1-t0);
#endif
...
static void wm_adsp2_boot_work(struct work_struct *work)
{
	...
	dsp->running = true;
	printk("arizona %s,%d,dsp->running=%d\n",__func__,__LINE__,dsp->running);
