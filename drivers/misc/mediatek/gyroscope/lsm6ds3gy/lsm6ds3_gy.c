...
static int recover_ST_gyro_reg_config(struct i2c_client  *client )
{
	...
	databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL4_C;
	...
	databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL5_C;
	...
	databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL6_C;
	...
	if(res <= 0)
	{
        ...
        databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL7_G;
	...
	if(res <= 0)
	{
        ...
        databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL8_XL;
	...
	databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL9_XL;
	...
	if(res <= 0)
	{
        ...
        databuf[1] = databuf[1] =  priv ->resume_LSM6DS3_CTRL10_C;
...
#ifdef LSM6DS3_CALIB_HAL
static DRIVER_ATTR(gyroscope_cali_val_x,      S_IRUGO | S_IWUGO, show_cali_gyrox_value, store_cali_gyrox_value);
static DRIVER_ATTR(gyroscope_cali_val_y,      S_IRUGO | S_IWUGO, show_cali_gyroy_value, store_cali_gyroy_value);
static DRIVER_ATTR(gyroscope_cali_val_z,      S_IRUGO | S_IWUGO, show_cali_gyroz_value, store_cali_gyroz_value);
#endif
/*lenovo-sw caoyi1 add for Gyroscope APK calibration 20150513 end*/
/*lenovo-sw caoyi1 add for selftest function 20150708 begin*/
#ifdef LSM6DS3_FTM_ENABLE
...
/*lenovo-sw caoyi1 modify A+G sensro iic address 20150417 begin*/
	//client->addr = 0xD4>>1;
	client->addr = 0xD5>>1;//modify for real iic addr
